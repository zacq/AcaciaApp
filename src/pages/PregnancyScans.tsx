...
import { Plus, Search, X, ChevronRight, Scan } from 'lucide-react';
...
type ScanResult = 'empty' | 'single' | 'twin' | 'triplet' | 'quad' | 'unknown';
...
const SCAN_RESULTS: ScanResult[] = ['empty', 'single', 'twin', 'triplet', 'quad', 'unknown'];

const resultBadge: Record<ScanResult, string> = {
  single:      'bg-green-50 text-green-700',
  twin:        'bg-emerald-50 text-emerald-700',
  triplet:     'bg-teal-50 text-teal-700',
  quad:        'bg-cyan-50 text-cyan-700',
...
export default function PregnancyScans() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setRecords((recRes.data as ScanRecord[]) ?? []);
  ...
  const filtered = search
    ? speciesRecords.filter(r =>
        ...
        r.scanner_name?.toLowerCase().includes(search.toLowerCase())
      )
    : speciesRecords;
  ...
  const openSheet = () => {
    setForm({
      animal_id: '', scan_date: format(new Date(), 'yyyy-MM-dd'),
      result: 'single', scanner_name: '', notes: '',
    });
    setError('');
    setShowSheet(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { data, error: err } = await supabase
      ...
      .insert({
        ...
        result: form.result,
        scanner_name: form.scanner_name || null,
    ...
    setRecords(prev => [data as ScanRecord, ...prev]);
    setShowSheet(false);
  };
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                <div className="relative pb-1">
                  ...
                  <input autoFocus value={search} onChange={e => setSearch(e.target.value)}
                    placeholder="Search by tag or scanner…" className="input-field pl-9 pr-9 py-2 text-sm" />
                  {search && (
      ...
      <div className="max-w-2xl mx-auto w-full px-4 pb-28">
        {loading ? (
          <table className="w-full text-sm mt-2"><tbody>
            {Array.from({ length: 10 }).map((_, i) => (
              <tr key={i} className="border-b border-accent/30">
                ...
                <td className="py-3 pr-3 w-20"><div className="h-5 w-14 bg-accent/30 rounded-full animate-pulse" /></td>
                <td className="py-3 hidden sm:table-cell"><div className="h-4 w-24 bg-accent/30 rounded animate-pulse" /></td>
          ...
          <div className="text-center py-20 border border-dashed border-accent rounded-[2rem] mt-4">
            <Scan size={40} className="mx-auto text-accent mb-3" />
            <p className="text-primary-light font-medium text-sm">
              {search ? 'No scans match your search' : `No pregnancy scans for ${activeSpecies.toLowerCase()}`}
            </p>
            {!search && (
              <button onClick={openSheet} className="mt-3 text-primary font-bold flex items-center gap-2 mx-auto text-sm">
                <Plus size={18} /> Record first scan
              </button>
            )}
          </div>
        ) : (
          <table className="w-full text-sm mt-2">
            <thead className="sticky top-[160px] z-10 bg-background">
              <tr className="border-b-2 border-accent text-[10px] uppercase tracking-wider text-primary-light/50 font-bold">
                ...
                <th className="py-2 text-left pr-3 w-20">Result</th>
                <th className="py-2 text-left hidden sm:table-cell">Scanner</th>
                <th className="w-5" />
              </tr>
            </thead>
            <tbody className="divide-y divide-accent/30">
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-accent/20 transition-colors">
                  ...
                  <td className="py-2.5 pr-3">
                    {r.result ? (
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${resultBadge[r.result]}`}>
                        {r.result}
                      </span>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-serif font-bold text-primary">New Pregnancy Scan</h3>
                ...
                <form onSubmit={handleSave} className="space-y-4 pb-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="date" required className="input-field" value={form.scan_date}
                    ...
                    <div className="col-span-2">
                      <label className="block text-xs font-bold text-primary-light uppercase tracking-wider mb-1">Scanner / Technician</label>
                      <input className="input-field" placeholder="Name of scanner or vet" value={form.scanner_name}
                        onChange={e => setForm({ ...form, scanner_name: e.target.value })} />
                    </div>
                    <div className="col-span-2">
                      <label className="block text-xs font-bold text-primary-light uppercase tracking-wider mb-1">Notes</label>
                      <textarea rows={2} className="input-field resize-none" placeholder="Additional scan notes…"
