...
import { Plus, Search, X, ChevronRight, Weight } from 'lucide-react';
...
export default function Performance() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setRecords((recRes.data as WeightRecord[]) ?? []);
  ...
  const openSheet = () => {
    setForm({
      animal_id: '', recorded_at: format(new Date(), 'yyyy-MM-dd'),
      weight_kg: '', age_days: '', condition_score: '', notes: '',
    });
    setError('');
    setShowSheet(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { data, error: err } = await supabase
      ...
      .insert({
        ...
        age_days: form.age_days ? Number(form.age_days) : null,
    ...
    setRecords(prev => [data as WeightRecord, ...prev]);
    setShowSheet(false);
  };
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                <div className="relative pb-1">
                  ...
                  <input autoFocus value={search} onChange={e => setSearch(e.target.value)}
                    placeholder="Search by tag or name…" className="input-field pl-9 pr-9 py-2 text-sm" />
                  {search && (
      ...
      <div className="max-w-2xl mx-auto w-full px-4 pb-28">
        {loading ? (
          <table className="w-full text-sm mt-2"><tbody>
            {Array.from({ length: 10 }).map((_, i) => (
              <tr key={i} className="border-b border-accent/30">
                ...
                <td className="py-3 hidden sm:table-cell"><div className="h-4 w-10 bg-accent/30 rounded animate-pulse" /></td>
          ...
          <div className="text-center py-20 border border-dashed border-accent rounded-[2rem] mt-4">
            <Weight size={40} className="mx-auto text-accent mb-3" />
            <p className="text-primary-light font-medium text-sm">
              {search ? 'No records match your search' : `No weight records for ${activeSpecies.toLowerCase()}`}
            </p>
            {!search && (
              <button onClick={openSheet} className="mt-3 text-primary font-bold flex items-center gap-2 mx-auto text-sm">
                <Plus size={18} /> Record first weight
              </button>
            )}
          </div>
        ) : (
          <table className="w-full text-sm mt-2">
            <thead className="sticky top-[160px] z-10 bg-background">
              <tr className="border-b-2 border-accent text-[10px] uppercase tracking-wider text-primary-light/50 font-bold">
                ...
                <th className="py-2 text-center w-12 hidden sm:table-cell">BCS</th>
                <th className="py-2 text-left hidden md:table-cell">Notes</th>
                <th className="w-5" />
              </tr>
            </thead>
            <tbody className="divide-y divide-accent/30">
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-accent/20 transition-colors">
                  ...
                  <td className="py-2.5 text-center text-xs text-primary-light hidden sm:table-cell">
                    {r.condition_score ?? '—'}
                  </td>
                  <td className="py-2.5 text-xs text-primary-light hidden md:table-cell">
                    {r.notes || '—'}
                  </td>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-serif font-bold text-primary">Record Weight</h3>
                ...
                <form onSubmit={handleSave} className="space-y-4 pb-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="date" required className="input-field" value={form.recorded_at}
                    ...
                    <div>
                      ...
                      <input type="number" required min="0" step="0.1" className="input-field" placeholder="e.g. 45.5"
                        value={form.weight_kg}
                        onChange={e => setForm({ ...form, weight_kg: e.target.value })} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-primary-light uppercase tracking-wider mb-1">Age (days)</label>
                      <input type="number" min="0" className="input-field" placeholder="e.g. 180"
                        value={form.age_days}
                        onChange={e => setForm({ ...form, age_days: e.target.value })} />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-primary-light uppercase tracking-wider mb-1">BCS (1–5)</label>
                      <input type="number" min="1" max="5" step="0.5" className="input-field" placeholder="e.g. 3.0"
